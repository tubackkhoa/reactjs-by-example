...
// this is a collection of saga watcher, then will be map like ... collection.map(f=>fork(f))
...
export const asyncFetchReducer = (state = initial_state, action) => {
	switch (action.type) {
		...
		// if we have sync_fetch_succeeded action type, then get data as fetch_result
